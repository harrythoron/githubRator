...
const validationSchema = yup.object().shape({
    mass: yup.number().min(1, 'Weight must be greater or equal to 1').required('Weight is required'),
    height: yup.number().min(0.5, 'Height must be greater or equal to 0.5').required('Height is required'),
});
...
const BodyMassIndexForm = ({ onSubmit }) => {
    ...
    return (
        <View>
            <TextInput placeholder="Weigth (kg)" value={formik.values.mass} onChangeText={formik.handleChange('mass')} />
